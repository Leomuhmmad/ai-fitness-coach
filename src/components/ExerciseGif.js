const exerciseGifs = {
  ...
  'diamond push-ups': 'https://gymvisual.com/img/p/1/5/3/0/1530.gif',
...
export default function ExerciseGif({ exerciseName, darkMode }) {
  ...
  const fallbackEmoji = () => {
    ...
    if (key?.includes('mountain') || key?.includes('bear') || key?.includes('crawl')) return '🔥'
    return '💪'
  }
  ...
  if (!gifUrl) return (
    <div style={{
      ...
      <div style={{ fontSize: 36 }}>{fallbackEmoji()}</div>
      <div style={{ fontSize: 11, color: darkMode ? '#AFA9EC' : '#7F77DD', fontWeight: 500 }}>{exerciseName}</div>
    </div>
  )

  return (
    <div style={{ width: '100%', borderRadius: 10, overflow: 'hidden', position: 'relative', background: '#000' }}>
      <img
        src={gifUrl}
        alt={exerciseName}
        style={{ width: '100%', height: 130, objectFit: 'cover', display: 'block', opacity: 0.95 }}
        onError={(e) => {
          e.target.style.display = 'none'
          e.target.parentNode.innerHTML = `<div style="width:100%;height:130px;background:${darkMode ? '#1E1B3A' : '#EEEDFE'};display:flex;align-items:center;justify-content:center;font-size:36px;border-radius:10px">${fallbackEmoji()}</div>`
        }}
      />
    </div>
  )
